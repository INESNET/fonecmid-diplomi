...
Процедура СформироватьДвиженияВзаиморасчетыСКлиентами();
	...
	Запрос.Текст =
		...
		|	ЕСТЬNULL(ВТ_ДопНачисления.Подразделение, ВТ_Удержания.Подразделение) КАК Подразделение,
		|	ЕСТЬNULL(ВТ_ДопНачисления.РезультатДоп, 0) - ЕСТЬNULL(ВТ_Удержания.РезультатУдерж, 0) КАК Результат
		|ИЗ
